pipeline {
    ...
    stages {
        ...
        stage('Monitoring') {
            ...
            bat '''
"C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -Command "try { $response = Invoke-WebRequest http://localhost:5000/health -UseBasicParsing; if ($response.StatusCode -eq 200) { Write-Host 'Health check passed' } } catch { Write-Host $_; exit 1 }"
'''
            }
        }
    }
